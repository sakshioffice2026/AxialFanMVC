...
namespace AxialFanMVC.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public ReportsController(AxialFanDbContext db) => _db = db;
        ...
        public async Task<IActionResult> Index(int? projectId = null, string? status = null)
        {
            ...
            var projectFilterOptions = await _db.Projects
                ...
                .OrderBy(p => p.Name)
                .Select(p => new ProjectFilterOption { Id = p.Id, Name = p.Name })
                .ToListAsync();
        ...
        public async Task<IActionResult> ExportPdf(int? projectId = null, string? status = null)
        {
            ...
            var pdfBytes = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().PaddingTop(10).Table(table =>
                    {
                        ...
                        {
                            c.RelativeColumn(2); // Project
                            c.RelativeColumn(2); // Application
                            c.RelativeColumn(1); // Media
                            c.RelativeColumn(1); // Flow
                            c.RelativeColumn(1); // Pressure
                            c.RelativeColumn(1); // Speed
                            c.RelativeColumn(1); // Blades
                            c.RelativeColumn(1); // Tip Dia
                            c.RelativeColumn(1); // Efficiency
                            c.RelativeColumn(1); // Shaft Power
                            c.RelativeColumn(1); // Safety Factor
                        ...
                        table.Header(header =>
                        {
                            void H(string text) => header.Cell().Background("#1864ab").Padding(3)
                                .Text(text).FontColor("#ffffff").Bold();

                            H("Project"); H("Application"); H("Media"); H("Flow (m³/s)");
                            H("ΔP (Pa)"); H("Speed (RPM)"); H("Blades"); H("Tip Dia (mm)");
                            H("Eff (%)"); H("Shaft (kW)"); H("SF"); H("Status"); H("Created");
                        });

                        foreach (var d in rows)
                        {
                            void C(string text) => table.Cell().BorderBottom(1).BorderColor("#dee2e6")
                                .Padding(3).Text(text);
                            ...
                            C(d.OverallEfficiencyPct.HasValue ? d.OverallEfficiencyPct.Value.ToString("F1") : "-");
                            C(d.ShaftPowerKw.HasValue ? d.ShaftPowerKw.Value.ToString("F2") : "-");
                            C(d.SafetyFactor.HasValue ? d.SafetyFactor.Value.ToString("F2") : "-");
            ...
            return File(pdfBytes, "application/pdf", $"AxialFan_Designs_Report_{DateTime.Now:yyyyMMdd_HHmm}.pdf");
        }
