...
Always verify with physical testing.
    ...
    //// ── POST /Export/PdfWithCharts ──────────────────────────────────
    //// Accepts chart snapshots captured client-side (exactly what the user
    //// sees on screen, including any pinned comparison curves) and embeds
    //// them into the PDF alongside the real Input/Aero/Structural data.
    //[HttpPost, ValidateAntiForgeryToken]
    //public async Task<IActionResult> PdfWithCharts([FromBody] PdfWithChartsRequest req)
    //{
    ...
    //    var (bytes, fileName) = _exportSvc.ExportPdfSummaryWithCharts(
